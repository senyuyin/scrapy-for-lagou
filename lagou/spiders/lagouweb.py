...
class LagouwebSpider(scrapy.Spider):
    ...
    for city in city_list:
        ...
        start_urls.append(position_url)
   ...
   #}
    ...
    def start_requests(self):
        for url in self.start_urls:
            yield FormRequest(url=url, formdata={'first':'true', 'pn':str(self.page), 'kd':'python 爬虫'}, callback=self.job_parse,dont_filter= True)
    ...
    def job_parse(self, response):
        ...
        if int(jdict['content']['pageNo']) != 0:
            ...
            yield FormRequest(url=response.url, formdata={'first':'false', 'pn':str(jdict['content']['pageNo']+1), 'kd':'python 爬虫'}, callback=self.job_parse, dont_filter= True)
